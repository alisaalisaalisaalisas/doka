...
- **[Песочница](docs/21-playground/index.md)** — симулятор терминала + редактор кода (Monaco) прямо на сайте: 126 сценариев по всем темам handbook'а. «Проверить решение» сверяет действия с целью; «Проверить код» — написанное в редакторе (Monaco).
- **[Тренажёр](docs/22-trainer/index.md)** — SRS-тренажёр в браузере (Anki-алгоритм: Снова/Хорошо/Легко, интервалы 1→70 дней, прогресс в localStorage). Карточки собираются автоматически из блоков «Проверь себя» всех разделов.
...
⚠️ В репозитории уже указан `repo_url` в mkdocs.yml — замените на свой перед деплоем.
