...
let srsStats = JSON.parse(localStorage.getItem(STATS_KEY) || "{}"); // {lastDay, streak, perDay: {day: count}}
function saveStats(){
  const today = new Date().toISOString().slice(0,10);
  if(srsStats.lastDay !== today){
    const yest = new Date(Date.now()-DAY).toISOString().slice(0,10);
    if(srsStats.lastDay === yest) srsStats.streak = (srsStats.streak||0)+1;
    else if(srsStats.lastDay) srsStats.streak = 1;
    else srsStats.streak = 1;
    srsStats.lastDay = today;
  }
  srsStats.perDay = srsStats.perDay || {};
  srsStats.perDay[srsStats.lastDay] = (srsStats.perDay[srsStats.lastDay]||0)+1;
  localStorage.setItem(STATS_KEY, JSON.stringify(srsStats));
}
function renderStreak(){
  ...
  for(const [day,cnt] of Object.entries(srsStats.perDay||{})){
  ...
  streakEl.textContent = `📊 Streak: ${srsStats.streak||0} дней · новых за неделю: ${weekNew} · к повторению: ${dueNow}`;
}
...
resetBtn.onclick = ()=>{ if(confirm("Сбросить весь SRS-прогресс?")){ state={}; history=[]; localStorage.removeItem(KEY); localStorage.removeItem(STATS_KEY); srsStats={}; shuffled=false; rebuild(); } };
