...
**В1. Универсальный каркас ответа на system design вопрос?**
<details><summary>Ответ</summary>
Уточнить требования (RPS, данные, SLA) → верхнеуровневая схема (LB → app → data) → узкие места и масштабирование каждого слоя → надёжность (репликация, failover) → наблюдаемость и деплой. Проговаривать trade-offs: «выбираю X потому что..., ценой Y».
</details>

**В2. Как диагностировать «приложение медленное» за первые 5 минут?**
<details><summary>Ответ</summary>
USE-обход: где именно медленно (client/server/network — трейс или тайминги)? Ресурсные лимиты (CPU throttling, память, диск IO, conntrack)? Свежие изменения (deploy/конфиг)? Зависимости (БД, внешние API)? Метрики p99 против среднего — хвостовые латентности прячут проблему.
</details>

**В3. Что важнее показать в разборе инцидента-кейса: скорость или системность?**
<details><summary>Ответ</summary>
Системность с быстрыми митигациями: сначала остановить ущерб (откат/флаг), параллельно собирать факты, потом root cause. Гонка за причиной при горящем проде — антипаттерн, который опытный интервьюер ловит специально.
</details>

**В4. Как оценить capacity системы, если дали мало данных?**
<details><summary>Ответ</summary>
Явно назвать допущения (профиль трафика, размер объекта) и считать от них: RPS × среднее время = число соединений, объём/сутки → диски/партиции, пиковый коэффициент ×2–5. Оценить запас 30–50%. Ценится сам расчёт с допущениями, а не «угаданная цифра».
</details>
